/**
 * This is a TypeGen auto-generated file.
 * Any changes made to this file can be lost when this file is regenerated.
 */

import { IReactProps } from "./IReactProps";
import { IReactComponentProps } from "./IReactComponentProps";
import { BaseReactComponentProps } from "./BaseReactComponentProps";

export interface AlternativeLoginBlockViewModel extends IReactProps, IReactComponentProps, BaseReactComponentProps {
  primaryButtonIntroduction?: string;
  primaryButtonText?: string;
  primaryButtonTarget?: string;
  secondaryLinkIntroduction?: string;
  secondaryLinkText?: string;
  secondaryLinkTarget?: string;
  activated: boolean;
}
