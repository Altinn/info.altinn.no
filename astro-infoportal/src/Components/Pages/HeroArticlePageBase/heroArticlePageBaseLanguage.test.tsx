...
const HeroArticlePageBase = (await import("./HeroArticlePageBase")).default;

// HeroArticlePageBase backs articlePage, sectionArticlePage and
// newsArticlePage. 57 of 70 news articles on at22 have no English variant, so
// their interface strings render inside a lang="nb" region (WCAG 3.1.2).
...
describe("HeroArticlePageBase last-updated byline", () => {
  ...
  it("renders the date alone when there is no label", () => {
    const markup = inEnglishChannel(
      <HeroArticlePageBase lastUpdatedDateString="18.09.2026" />,
    );

    expect(markup.textContent).toContain("18.09.2026");
  });

  // The article's own title is content, not interface text.
  it("leaves the editor-written page name alone", () => {
    const markup = inEnglishChannel(
      <HeroArticlePageBase
        pageName="Nye regler for skattemelding"
        lastUpdatedDateText="Last updated"
        lastUpdatedDateString="18.09.2026"
      />,
    );

    expect(langOf(markup, "Nye regler for skattemelding")).toBeUndefined();
  });

  it("leaves the editor-written intro alone", () => {
    const markup = inEnglishChannel(
      <HeroArticlePageBase mainIntro="Dette gjelder fra nyttår." />,
    );

    expect(langOf(markup, "Dette gjelder fra nyttår.")).toBeUndefined();
  });
});

describe("HeroArticlePageBase on a properly translated page", () => {
