...
interface IReactComponentPropsType {
  type: string;
  items: IReactComponentProps[];
  startIndex: number;
}

const ContentArea = ({ items }: ContentAreaProps) => {
  const itemListOfLists: IReactComponentPropsType[] = [];
  for (let i = 0; i < items.length; i++) {
    const currentItem = items[i];
    const newItems = [] as IReactComponentProps[];
    const startIndex = i;
  ...
  const resolveDisplayClass = (item: IReactComponentProps): string => {
